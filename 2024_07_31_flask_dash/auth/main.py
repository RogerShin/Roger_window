...
from .datasource import validateUser, inster_data, InvalidEmailException
...
class UserRegistrationForm(FlaskForm):
    ...
    uEmail = EmailField('郵件信箱', validators=[DataRequired()])
...
def register():
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            uBirthday:datetime.data | None = form.uBirthday.data
            if uBirthday is not None:
               uBirthday_str = uBirthday.strftime("%Y-%m-%d")
               print("出生年月=", uBirthday_str)
            else:
            ...
            try:
                inster_data([uName, uGender, uPhone, uEmail, isGetEmail, uBirthday_str, uAboutMe, hash_password, conn_token])
